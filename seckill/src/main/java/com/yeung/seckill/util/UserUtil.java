...
public class UserUtil {
    public static void create(int count) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            User user = new User();
            user.setId(13158707563L + i);
            user.setNickname("user" + i);

            // 用户数据表的slat,由程序员设置
            user.setSlat("ptqtXy16");
            // 前端的salt = 3Tz7EFoh
            // var str = "" + salt.charAt(0) + inputPass + salt.charAt(4)
            String temp = "3" + "12345" + "E";
            user.setPassword(MD5Util.midPassToDBPass(temp, user.getSlat()));
            users.add(user);
        }
        ...
        File file = new File("D:\\YeungLaw\\temp\\config.txt");
        if (file.exists()) {
            file.delete();
        }
    ...
    private static Connection getConn() throws Exception {
        String url = "jdbc:mysql://8.138.161.55:3306/seckill?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai";
        String username = "seckill";
        String password = "seckill";
